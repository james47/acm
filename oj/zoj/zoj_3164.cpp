...
void multi(int w, int v, int t, int dp[]){
	int maxl = t * w;
	for (int j = 0; j < w; j++){
		head = tail = 0; 
		for (int k = j, cnt = 0; k <= D; k += w, cnt++){ 
	...
	while(scanf("%d %d", &N, &D) != EOF)
	{
		...
		for (int i = 1; i <= N; i++){
			...
			if (k[i] == 0 || k[i] * p[i] >= D)
				...
				zero_one(p[i], e[i], flag[i] ? tmp : dp);
			}
			else{
				multi(p[i], e[i], k[i], flag[i] ? tmp : dp);
			}
